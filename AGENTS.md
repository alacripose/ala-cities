...
Issues and specs live as GitHub issues, driven by the `gh` CLI (the plan's wayfinder map is a `wayfinder:map` issue with child tickets). See `docs/agents/issue-tracker.md`.
...
Single-context: `CONTEXT.md` at the root plus `docs/adr/`, created lazily by `/domain-modeling`. See `docs/agents/domain.md`.