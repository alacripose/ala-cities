...
**Current implementation state.** `src/asset_generator.rs` now has two explicit outputs: `build_chunk` reads `&FoundingWorld`, builds a sparse octree plus deterministic axis-aligned greedy masks emitted as indexed triangles, and caches by seed/revision/chunk/chunk-builder recipe plus a versioned `ChunkInputDigest`; that digest covers the resident chunk and the residency/inward-facing boundary voxels of its six cardinal neighbours, including typed voxel mass and the existing textual voxel-kind identity. Greedy integration tests reconstruct every authoritative visible unit face and material for positive and negative chunks, reject overlaps, require a substantial triangle reduction, and verify outward winding against the declared normal on all six signed directions. A named `FoundingWorld::cubic_preview` fixture now proves a complete 32³ section: 32,768 voxels, 12 indexed hull triangles, camera culling, and a valid native camera ray. The fixture is explicitly not canonical generator truth. Both chunk and shape outputs carry versioned OpenPBR-ready manifests. Every current world/gear material is explicitly opaque: base weight and alpha are exactly `1.0`, transmission is exactly `0.0`, and blend mode is opaque; contradictory optical declarations fail manifest construction, and semantic recipe tweaks cannot change the material manifest. `build_shape` derives a candidate `BuilderHash` from quantized Teeth/Opening/Accent values and builds one coherent indexed gear mesh. `src/asset_controls.rs` implements three separate screen-space slider rows with orb handles, pointer mapping, 48 px targets, and keyboard nudges. `src/bin/ala-cities-target.rs` renders the cubic preview through the real runtime chunk path and exact octree-backed ray read-back without authoring UI. `src/bin/asset-builder.rs` is the external semantic authoring/review process. `src/octree.rs` and `src/raycast.rs` retain sparse occupancy and exact signed traversal; the octree now reports exactly 37,449 nodes for a full 32³ tree. `src/asset_streaming.rs` adds deterministic tick-budgeted staging over resident signed chunks with duplicate-request rejection, world-digest preservation, and least-recently-built eviction under explicit cache pressure. The release benchmark records a nine-chunk, one-build-per-tick schedule but does not yet integrate concurrent I/O or LOD. The C2ME/CubicChunks architectural references and their MIT provenance boundary are recorded in `docs/research/cubic-chunk-references.md` and `ATTRIBUTION.md`. This is meaningful partial evidence for Q744/Q747/Q751/Q856–Q858. It still does not evidence full generator/source/table digests, a canonical whole-world projection identity, promoted OpenPBR texture assets, streaming/LOD policy, measured performance, completed human promotion, or replacement of the legacy client.

**Remaining questions, owner, and exact next evidence.** Owners: [Dynamic asset builder and chunk mesher boundary](https://github.com/alacripose/ala-cities/issues/22) and [Generator performance and first-gate evidence](https://github.com/alacripose/ala-cities/issues/28). Next evidence: generator/source-digest change, whole-world projection invalidation beyond the current seven-chunk input, recipe/hash reproducibility, promoted OpenPBR texture/reference assets, runtime integration plus concurrent I/O and LOD correctness, derived build/frame budgets across cold processes and hardware, and mechanical+human promotion records.
