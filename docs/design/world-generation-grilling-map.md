...
| A-15 | Current builder cache identity matches the Q751 fields named in code, but mutable world deltas are not part of the key. | Q751/Q856 require the builder to consume current world truth; cache correctness must include enough authoritative chunk/projection identity to invalidate changed state. | Add a world/projection/chunk-state digest or explicit invalidation dependency and prove it. |
