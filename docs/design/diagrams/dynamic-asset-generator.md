...
The generator is required target infrastructure. It owns the runtime chunk presentation path, including sparse acceleration, materials, manifests, greedy masks emitted as indexed triangles, and conservative camera culling. The runtime renderer consumes validated manifest colors rather than duplicate UI constants, so UI tint cannot alter optics. The renderer is the game surface, not a builder or authoring surface. Its separate exact voxel ray-picking path is read-only game observation and returns hit evidence; it is not a world mutation or an authoring control.

**Current meshing and cache grounding.** `src/asset_generator.rs` keys the runtime chunk cache by seed/revision (the implemented generator identity), chunk coordinates, the chunk-builder recipe, and a versioned `ChunkInputDigest`. The digest covers the resident chunk plus the residency and inward-facing boundary voxels of its six cardinal neighbours. This preserves the seven-chunk presentation-input boundary; it is not full generator/source/table identity or a canonical whole-world projection digest. `tests/greedy_mesh.rs` reconstructs every emitted indexed triangle as unit faces and compares the exact face-and-material map against authoritative `FoundingWorld` visibility for both the positive chunk `ChunkCoord::new(0, 0, 0)` and the negative chunk `ChunkCoord::new(-1, 0, 0)`, rejecting overlap. It verifies that indexed triangles preserve exact outward winding across all six signed face directions, and a separate assertion requires the triangle count to be less than half the visible unit-face count. `tests/cubic_chunk_preview.rs` uses the named `FoundingWorld::cubic_preview` fixture: one full 32³ section, 32,768 voxels, and 12 hull triangles. This fixture proves preview occupancy, indexed meshing, and camera culling; it is not a claim that canonical generator truth is a solid cube. The preview follows the licensed C2ME/CubicChunks architectural references recorded in `docs/research/cubic-chunk-references.md`; no upstream source has been copied yet.
...
- The amber unresolved strip deliberately preserves open work: full generator/source/table identity beyond the current seven-chunk presentation input, streaming/LOD, measured build/frame performance, promoted OpenPBR textures, mechanical/human candidate promotion, the reference browser, and bronze release/playtest evidence. The implemented versioned basic manifests and licensed architectural references do not close those future authoring, identity, performance, or review items.
...
**Implementation evidence:** `tests/cubic_chunk_preview.rs`, `tests/greedy_mesh.rs`, `tests/view_culling.rs`; **reference record:** `docs/research/cubic-chunk-references.md`.
